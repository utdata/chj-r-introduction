...
This first day -- only an hour, really -- we'll be learning about RStudio, RMarkdown and how to use them. I promise we'll get to to the data.
...
We also want to install another package we'll be using soon, called [janitor](https://cran.r-project.org/web/packages/janitor/vignettes/janitor.html).

- In your **Console**, run `install.packages("janitor)`.
...
- Use `head()` to view the top of the created dataframe "opioid".

```{r explore, exercise=TRUE, exercise.lines = 5, exercise.setup = "prepare-opioid"}
