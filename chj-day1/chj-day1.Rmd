...
What you've done here is create a plot chart of sample data "cars" that is already embedded inside the R programming language. (FWIW, it is the speed of cars and the distances taken to stop. Note that the data were recorded in the 1920s.) There are other data sets like this embedded in R for examples like this.
...
- Run the code below so you see what happens when we import data.

```{r opioid-read, exercise=TRUE}
...
```{r opioid-clean, exercise=TRUE, exercise.lines = 10}
# import the data
deaths <- read_csv("https://github.com/utdata/chj-r-introduction/blob/main/data/chj_mn_opiate_deaths_data.csv?raw=true")
...
```{r explore-summary, exercise=T, exercise.setup = "prepare-explore"}
deaths %>% summary()
```
...
```{r explore-glimpse, exercise=T, exercise.setup = "prepare-explore"}
deaths %>% glimpse()
```
