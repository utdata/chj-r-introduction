...
```{r deaths-select, exercise=T, exercise.setup="prep-deaths", exercise.lines=6}
# all columns in deaths
deaths
...
The first return is all the columns in the dataframe. The second return is just the columns we named within the `select()` function
...
```{r deaths-select-endswith, exercise=T, exercise.setup = "prep-deaths"}
deaths %>% 
...
Read and Run the code chunk below to see how `filter()` is used. (I add the `select()` and `head()` functions to more clearly see the result.

```{r deaths-filter, exercise=T, exercise.setup = "prep-deaths"}
deaths %>% 
  filter(gender == "M") %>% 
...
Note that to match a specific value in the `filter()` equation, you need two equal signs: `==`. Here are the other logical tests you can use:
...
- Write a filter to get only records where the `age_years` is less than 18 and then select only the name and age columns in the result. We've started you off with the `deaths` data frame.

```{r try, exercise=T, exercise.setup = "prep-deaths", exercise.line = 8}
deaths
```
...
```{r filter-multi, exercise=T, exercise.setup = "prep-deaths", exercise.lines=6}
deaths %>% 
  filter(
    gender == "M",
...
```{r filter-or, exercise=T, exercise.setup="prep-deaths"}
deaths %>% 
...
I'll show you one more because I find it particularly useful: Finding rows within a list of values. This gets the same result as the code above, but is easier to write and understand.

```{r filter-in, exercise=T, exercise.setup="prep-deaths"}
deaths %>% 
...
```{r arrange, exercise=T, exercise.setup="prep-deaths"}
deaths %>% 
  arrange(age_years) %>% 
  select(age_years)
```
...
```{r arrange-desc, exercise=T, exercise.setup="prep-deaths", exercise.lines=6}
deaths %>% 
  # arrange descending order using the tidyverse pipe
  arrange(age_years %>% desc()) %>% 
  # select just to see the result. And opioid death at 98?
...
We'll use `mutate()` to convert our text dates to a real datetime object. We'll be using the [lubridate](https://lubridate.tidyverse.org/) package to help us, but we installed it yesterday and included it in the libraries at the top of our project.

### Using mutate to convert dates

We'll show this first in this learning environment before fixing the data in your project.

```{r convert-date, exercise=T, exercise.setup="prep-deaths", exercise.lines=8}
deaths %>% 
  mutate(
...
```{r mtcars-math, exercise=T, exercise.lines=6}
mtcars %>% 
  mutate(
    hp_cyl= hp/cyl
  ) %>% 
...
In your own project, use the convert dates example above to write a mutate rule to convert all three date columns in `deaths` into new "_clean_" columns, but  **assign the result back into the `deaths` dataframe**. _After_ you've reassigned the data frame, start again with `deaths` and pipe into a `select()` to show all six date columns so you can compare them.
...
```{r convert-date-practice, exercise=T, exercise.setup="prep-deaths", exercise.lines=10}
...
BORING TIDBIT: Hadley Wickham is from New Zealand, so there are both `summarise()` and `summarize()` functions, but they do exactly the same thing. Unfortunately for Americans, summarise arrives first in alphabetical order in type assist.
...
Read and run the code and then we'll walk through it line-by-line.

```{r group-mean, exercise=T, exercise.setup="prep-deaths", exercise.lines=6}
deaths %>% 
  group_by(gender) %>% 
  summarize(
    avg =  mean(age_years),
...
You shouldn't mix up your nesting/tidyverse style like this, but I wanted to who you both methods. In this case, the nested style _might_ be a little easier to understand, but I like the consistency of using tidyverse style.
...
- In the `group_by` I'm grouping by two values, the "year of death" and `race`.
  ...
  - The second grouping value is `race`. This means the data is organized by all our 2005 African American rows first, then 2005 American Indian rows, etc., until we get to the next year.
...
- Get the average age by hispanic_ethnicity. This will take a traditional `group_by()` and `summarize()` combination.
- Get counts by year. You can use `count()`, but you'll have to parse the death_clean_date to group by the year.
