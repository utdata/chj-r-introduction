...
We can't show how this works interactively, so we'll just have you set this up and run it locally.
...
1. Add a chunk and load libraries for tidyverse, readxl, janitor.
...
- The data is in a sheet names **Results**. We have to specify that sheet or we'll get the **Summary** sheet by default.
...
- You'll see here we are importing the **Summary** tab, which doesn't work for us.

### In RStudio: Add the sheet value

1. **Edit your chunk** to add sheet argument like this. **Don't forget the comma after the path line**:
...
The first called R Console notes that there are problems. Here is a bit of it:
...
This is telling us that the first column has errors, though I admit it is not very telling. What is wrong is the "Arrest Primary Key" is being considered a number, but some values have a dash in it, which is text. We will need to fix that.

The second tab is the table that is returned. It did import, just with the problems noted above.

If you tab through the columns in the table, you'll note another issue: **The ZIP is imported as a number**. Not a big issue in Texas, but a real problem for ZIPs that start with zero. We need to change that to text. Always convert ZIPs to text. We'll do that when we fix the arrest ID.
...
1. **Edit your chunk** to add a the the comma, the col_types arguments and the glimpse at the end so we can see our results.
...
The glimpse allows us to see the results in an easy way. We'll have to remove that later.
...
As noted earlier, there are other packages like that read in data of different types and from different sources.
...
We've put together some exercises to practice importing and exporting data.